...
class DiscoverTabView extends StatelessWidget {
                ...
                child: Obx(() {
                  if (discoverController.isVideoRoomMode) {
                    return DiscoverVideoRoomView(
                      rooms: discoverController.demoVideoRooms,
                      onCreateVideoRoom: discoverController.openCreateVideoRoom,
                      onJoinLive: (room) =>
                          _showJoinVideoRoomPreview(context, room),
                    );
                  }
  ...
  Future<void> _showJoinVideoRoomPreview(
    BuildContext context,
    Map<String, dynamic> room,
  ) {
    final roomName = room['name']?.toString() ?? 'Video Room';
    final hostName = room['hostName']?.toString() ?? 'Host';
    final viewers = room['viewerCount']?.toString() ?? '0';
    final seats = '${room['maxSeats'] ?? 8}';

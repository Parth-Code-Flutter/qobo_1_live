...
class AgencyHostListView extends GetView<AgencyHostListController> {
  ...
  Widget _hostApplicationsList(
    ...
    final hosts = ctrl.hostsForTab(pending: pending);
          ...
          return Padding(
            ...
            child: Obx(
              () => _HostSheetCard(
                ...
                isProcessing: ctrl.processingReviewId.value.isNotEmpty,
                onApprove: () => ctrl.approveHostApplication(host),
                onReject: () async {
                  final reason = await showAgencyHostRejectReasonDialog(
                    context,
                  );
                  if (reason != null && reason.isNotEmpty) {
                    await ctrl.rejectHostApplication(host, reason);
                  }
                },
